...
$(document).ready(function () {
	var markers = [],
		...
		border = new google.maps.Polygon({
			paths: [
				...
				google.maps.geometry.encoding.decodePath(pl_path)
			],
			fillColor: "#FFFFFF",
			fillOpacity: .8,
			strokeOpacity: .5,
		...
		pl_bounds = new google.maps.LatLngBounds(new google.maps.LatLng(49.00238, 14.12298), new google.maps.LatLng(54.833333, 24.14585)),
		...
		Localizer = Class.extend({
			...
			request_position: function () {
				if (this.nav) {
					...
					if (this.geoloc) {
						alerts(mPHeart.translation.LC_FINANSE_POSITION_LOADING, 'alert-info alert-lookingPosition');
			...
			request_position_success: function (position) {
				mapGeodecode(position.coords.latitude, position.coords.longitude);
			},
	...
	function mapInit() {
		...
		/*
		$('form.suggesterBlock').submit(function (e) {
		...
		*/
		...
		var marker_komisja = new google.maps.Marker({
			position: {lat: 52.227601, lng: 21.015094},
			map: map,
	...
	function mapPlace(place) {
		...
		var main = $('.dataBrowserContent'),
			...
			dtlWnd = $('<div></div>').addClass('mapaDetailWindow').append(
				$('<ul></ul>').addClass('title')
			).append(
				$('<ul></ul>').addClass('info').addClass('adm')
			).append(
				$('<ul></ul>').addClass('info').addClass('wybory')
			).append(
				$('<ul></ul>').addClass('info').addClass('komisja')
			);

		if (typeof place.title !== undefined) {
			dtlWnd.find('ul.title').append(
				$('<li></li>').append(
					$('<a></a>').attr({
						'href': '#',
						'target': 'blank'
		...
		*/
		...
		if (typeof data['miejsca.gmina_id'] !== undefined) {
			dtlWnd.find('ul.info.adm').append(
				$('<li></li>').append(
		...
		*/
		...
		dtlWnd.find('ul.info.wybory').append(
			$('<li><h3>Na kogo głosować?</h3></li><li><button class="btn btn-primary btn-xs">Pokaż kandydatów do Sejmu &raquo;</button></li><li><button class="btn btn-primary btn-xs">Pokaż kandydatów do Senatu &raquo;</button></li>')
		);
		
		dtlWnd.find('ul.info.komisja').append(
			$('<li><h3>Gdzie głosować?</h3></li><li class="komisja-adres">Ministerstwo Rolnictwa i Rozwoju Wsi, ul.Wspólna 30 , Śródmieście 00-519 Warszawa</li>')
		);
		...
		var pbounds = new google.maps.LatLngBounds(new google.maps.LatLng(place.viewport.top_left.lat, place.viewport.top_left.lon), new google.maps.LatLng(place.viewport.bottom_right.lat, place.viewport.bottom_right.lon));
		map.fitBounds(pbounds);
		
		for( var i=0; i<place.points.length; i++ ) {
			
			var p = place.points[i];
	...
	function mapGeodecode(lat, lon) {
		...
		$.ajax({
			...
			success: function (res) {
				...
				// console.log('res', res);
				window.location = '/mapa/miejsce/253272?n=25';
				// window.location = '/mapa/miejsce/' + res['data']['miejsca.id'];
