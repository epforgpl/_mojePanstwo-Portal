/*global pl_number_format,$,Class,Geohash,document,window,google*/
...
var MapBrowser = Class.extend({
	...
	load: function () {
		...
		// apply actual center and zoom to app's chapters hrefs + url hash
		var param = $.param(q);
	...
	showPlaces: function (mode, data, layer) {
		...
		if (mode === 'adres' || mode === 'warstwy_adres') {
			...
			if (self.dataLoaded.points && self.dataLoaded.layers) {
				...
				for (i = 0; i < points.length; i++) {
					...
					for (j = 0; j < points[i]['punkty'].length; j++) {
						...
						this.markers.adres['p' + p['id']] = new CustomMarker(self, new google.maps.LatLng(p['lat'], p['lon']), self.map, {
							mode: 'adres',
