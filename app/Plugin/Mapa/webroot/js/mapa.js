	...
	var map,
		...
		options = {
			...
			request_position_success: function (position) {
				mapRequest(position.coords.latitude + ' ' + position.coords.longitude);
			},
		...
		if (q.length > 0) {
			$.ajax({
				...
				success: function (res) {
					$.each(markers, function (value) {
						value.setMap(null);
					});
					markers = [];

					$.each(res.places, function () {
						var marker = new google.maps.Marker({
							position: new google.maps.LatLng(this.locations[0].location.lat, this.locations[0].location.lon),
							map: map,
							data: this.data
						});
