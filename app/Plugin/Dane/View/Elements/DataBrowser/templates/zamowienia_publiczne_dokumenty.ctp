...
} elseif( $object->getData('wartosc_cena') ) {
	
	$metaDesc[] = number_format_h( $object->getData('wartosc_cena') ) . ' PLN';
	
	if( 
		( $static = $object->getStatic() ) && 
		isset($static['wykonawcy']) && 
		( $count = count($static['wykonawcy']) )
	) {
		
		$d = $this->Text->truncate($static['wykonawcy'][0]['nazwa'], 40);
		
		if( $count>1 )
			$d .= ' i ' . pl_dopelniacz($count, 'wykonawca', 'wykonawców', 'wykonawców');
            ...
            <div>
                ...
                <div class="content<? if ($object->getPosition()) { ?> col-md-11<? } ?>">
                    ...
                    <div class="<? if ($object->getIcon()) { ?>object-icon-side <? } ?>">
                        ...
                        <p class="meta meta-desc"><?= implode(' <span class="separator">-</span> ', $metaDesc) ?></p>
