...
Okregi.prototype.initialize = function() {
    ...
    $('button.okrag')
        .bind('mouseenter', function() {
            ...
            _this.markOkregiPolygonAsHover(index);
        })
        .bind('click', function() {
            var el = $(this);
            var index = el.attr('data-index');
            _this.clickOnOkrag(index);
        });
};
...
Okregi.prototype.createAndAddToMapOkregiPolygons = function() {
    for(var i = 0; i < this.data.length; i++) {
        if(this.data.hasOwnProperty(i)) {
            ...
            google.maps.event.addListener(this.data[i].polygon, 'click', function() {
                _this.okregiPolygonClick(this);
            });
        }
    }
};

Okregi.prototype.okregiPolygonClick = function(polygon) {
    this.clickOnOkrag(polygon.i);
};
...
Okregi.prototype.okregiPolygonMouseOver = function(polygon) {
    this.markOkregiPolygonAsHover(polygon.i);
};
