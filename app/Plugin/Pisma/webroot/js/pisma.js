...
var PISMA = Class.extend({
    ...
    editor: function () {
        ...
        if (self.html.editor.length) {
            tinymce.init({
                language: 'pl',
                selector: '#editor',
                content_css: '/css/main.css,/Pisma/css/pisma_render.css',
                toolbar: "undo redo | bold italic underline | blackquote | bullist numlist outdent indent",
                menubar: false,
                statusbar: false
            });
            ...
            if (that.hasClass('date')) {
                ...
                if (that.hasClass('email')) {
                    ...
                    that.find('input.kwota').keyup(function () {
                        ...
                        /*TODO: reenable it*/
                        tinyMCE.activeEditor.getDoc().documentElement.innerHTML.find('.slownie[data-unique="' + $(this).parent().data('unique') + '"]')
    ...
    generateFormInsert: function () {
        ...
        preview.find('.content').html(tinyMCE.activeEditor.getContent());
        ...
        preview.append(
            $('<textarea></textarea>').attr({name: 'tresc_html'}).val(tinyMCE.activeEditor.getContent())
        );
        ...
        preview.append(
            $('<textarea></textarea>').attr({name: 'tresc'}).val($.trim(tinyMCE.activeEditor.getDoc().documentElement.innerHTML))
        ).append(
